...
6. **Optional social + FAQ rich results (Tree-approved, 2026-05-02 — Nova shipped):** Per-route **`openGraph`** + **`twitter`** on **`/`** and the five SEO targets (`/reflection-ai`, `/self-reflection-app`, `/reflection-without-advice`, `/journaling-alternative`, `/faq`) using the **same** title/description as each page (`lib/wisewave-site/wisewave-marketing-social-metadata.ts`). Default **`og:image`** / **`twitter:image`**: **`/brand/wisewave-app-logo.png`** (`lib/wisewave-site/wisewave-default-share-image.ts`; also root **`app/layout.tsx`**). **`/faq`** FAQPage **JSON-LD** + accordion share **`lib/wisewave-site/wisewave-marketing-faq-items.ts`** (visible FAQ = structured data only). **Lumen JSON-LD layer (2026-05-02):** marketing **`layout`** adds **`Organization` + `WebSite`** @graph (`MarketingSiteWideJsonLd`); SEO landings + **`/faq`** add **`BreadcrumbList`** (leaf name = visible H1). Post-deploy: **Rich Results Test** on `/faq`; view-source for **`@type`** and FAQ parity; spot-check **`og:image`** on marketing URLs.
