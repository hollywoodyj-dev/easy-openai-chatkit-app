...
- **2026-07-17 Lumen P1 legibility Preview QA:** **HOLD for hosted Preview signoff.** Local passes; Production guard clean on `3a228eb` deploy (`www.wisewave.io/chat` — no P1 block). **Blocker unchanged:** `main` push → Vercel `target: production` (deploy `6e3grdxae`, 15:36 AEST); no `target: preview` deploy for `3a228eb`; deployment URLs behind Vercel Login; no bypass secret for Lumen. **Steward must:** (1) Preview-scoped env `NEXT_PUBLIC_ENABLE_P1_INTERACTION_LEGIBILITY=1`, (2) a **Preview** deployment URL (PR/branch — not production), (3) Vercel Protection bypass shared securely. Artifact `docs/qa/P1_INTERACTION_LEGIBILITY_PREVIEW_SLICE_LUMEN_QA_2026-07-17.md`.
