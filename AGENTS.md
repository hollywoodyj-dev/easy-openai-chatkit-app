...
| Wisewave libs | `lib/wisewave-*.ts`, `lib/wisewave-milestone-f-embodiment.ts`, `lib/wisewave-milestone-g-integration.ts`, `lib/wisewave-milestone-h-micro-awareness.ts` (H cue engine), `lib/wisewave-milestone-h-light-mode.ts` (Wisewave Light Mode v2 → system appendix); Milestone **J** (when implemented): narrow hook in existing turn path per J addendum — no new UI / persistence |
...
- Milestone J: addendum `docs/HC_OS_V1_Milestone_J_Addendum_Micro_Shift_Embodied_Effect_Layer.md`; OctopusMind boundary `docs/HC_OS_V1_Milestone_J_OctopusMind_Boundary_Handoff.md` (Stream 2 QA / implementation alignment); Lumen execution artifacts TBD once Stream 4 runs
