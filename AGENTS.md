...
| Phase 5 adoption instrumentation | **`app/api/chat/threads/route.ts`** + **`app/api/chat/turn/route.ts`** — lightweight debug fields for Continue journey tracing (`selected_at`, path stage flags) to support Phase 5 dependency-layer observation without changing user-facing UI. |
