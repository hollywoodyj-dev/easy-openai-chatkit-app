...
export const PRODUCT_CONTINUITY_RE =
  /\b(this )?reflection\b|\bsaved reflection\b|\bthis note\b|\bnote stays saved\b|\baccount\b|\bbrowser\b|\bprivate (browsing|mode)\b|\bwithout (an )?account\b|\bleave (this|what you said|what you wrote) here\b|\bkeep this reflection\b|\bsave this reflection\b|\breturn to (this )?reflection\b|\bcome back to this reflection\b|\bremains in your account\b|\bstays saved in your account\b|\bbegin with what is present\b|\bcontinue from (where|the place)\b|\ba line (you |to |you chose|you saved)\b|\bacross devices\b|\bleave without saving\b|\bcontinue without\b|\bremain available\b|\bwill (still )?be here if you return\b|\baccount settings\b|\bstay open\b|\baccess this reflection\b|\b(?:saved|session|the|your|this)\s+(?:checklist|outline|transcript|worksheet|article|notes?|voice\s+memo|draft)\b|\bsupport\s+(?:queue|ticket)\b|\banchor\s+link\b|\b(?:the\s+)?(?:reminder|notification|status\s+page|autosaves?|exported\s+(?:copy|transcript))\b|\bin\s+the\s+archive\b|\bmessage\s+thread\b|\boriginal\s+ticket\b|\baudit\s+service\b|\brecovery\s+email\b|\bvoice\s+locator\b|\bsaved\s+recording\b|\bwelcome\s+banner\b|\bclose\s+button\b|\bversion\s+control\b|\bname\s+resolver\b|\bcollect\s+job\b|\bcompass\s+widget\b|\bmap\s+view\b|\bdisplay\s+name\b|\bdirectory\s+service\b|\bhem\s+tool\b|\bhush\s+mode\b|\bface\s+store\b|\bstep\s+detector\b|\breply\s+audit\b|\bencrypted\s+vault\b|\bcalendar\s+link\b|\bidentity\s+verifier\b|\bpalm\s+sensor\b|\bsilence\s+detector\b|\bdeparture\s+log\b|\bname\s+service\b|\bweek\s+view\b|\bsigned\s+handle\b|\b(?:this|the\s+saved)\s+section\b|\bin\s+the\s+library\b|这段反思|这条记录|已保存的(?:清单|大纲|笔记|练习表|文档|录音)|语音笔记|资料库|这份草稿|支持工单|锚点链接|原段落|消息线索|原始工单|审计服务|恢复邮箱|语音定位器|欢迎横幅|关闭按钮|指南针控件|地图视图|版本服务|名称解析器|收集任务|目录服务|卷边工具|静音模式|面孔库|步态检测器|显示名|归档|回复审计|加密保管库|日历链接|身份验证服务|掌心传感器|静默检测器|离开日志|置顶卡片|名称服务|周视图|静音标记|黎明任务|健康进程|保存在账户|账户里?会保留|账户|浏览器|不注册|不保存|留存这段|会保留|你可以先把它留在这里|稍后再回到这段|从今天此刻|接着上次|想留给下次|为自己留下的一句话|如果你想以后再回来/i;
...
function cleanProductFragment(text: string): string {
  let out = text
    .replace(/\s{2,}/g, " ")
    .trim();
  // Strip leading concessive / coordinating connectors left after clause removal.
  out = out
    .replace(/^(?:although|though|even\s+though|while|whereas|but|yet|and|so)\s+/i, "")
    .replace(/^(?:同时|但是|但|然而|即使|虽然|尽管|而)\s*/u, "")
    .trim();
...
export function rewriteMixedRemovePersonal(
  ...
  const connectorParts = working
    .split(/\s*(?:[,，]|[；;]\s*|\s+-\s+|\s+而\s+|\s+and\s+|\s+while\s+|\s+whereas\s+|\s+but\s+|\s+yet\s+|同时|但是?|却)\s*/u)
    .map((s) => cleanProductFragment(s))
    .filter(Boolean);
